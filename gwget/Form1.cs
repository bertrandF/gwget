...
namespace gwget
{
    ...
    public partial class Form1 : Form
    {
        ...
        private bool validateForm() {
            ...
            if (MethodComboBox.Text.Length <= 0) {
                MessageBox.Show("You must specify a method", "Bad Entry");
                return false;
            }

            if (FileTextBox.Text.Length <= 0) {
                MessageBox.Show("You must specify a file name", "Bad Entry");
                return false;
            }
        ...
        private void ReceiveCallback(IAsyncResult ar)
        {
            try
            {
                ...
                if (bytesRead > 0)
                {
                    ...
                    for (int i = 0; i < StateObject.BufferSize; ++i)
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            UriBuilder uriBuilder = new UriBuilder("http", HostnameTextBox.Text, (int)PortNumericUpDown.Value);
            if (FileTextBox.Text.IndexOf('?') < 0) 
            {
                uriBuilder.Path = FileTextBox.Text;
            }
            else
            {
                uriBuilder.Path = FileTextBox.Text.Substring(0, FileTextBox.Text.IndexOf('?') + 1);
                uriBuilder.Query = FileTextBox.Text.Substring(FileTextBox.Text.IndexOf('?')) ?? "";
            }
